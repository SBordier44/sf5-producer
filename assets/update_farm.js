...
class Address {
    ...
    get country() {
        return this.address_components.find(data => data.types.indexOf("country") >= 0).long_name
    }

    get region() {
        return this.address_components.find(data => data.types.indexOf("administrative_area_level_1") >= 0).long_name
    }

    get department() {
        return this.address_components.find(data => data.types.indexOf("administrative_area_level_2") >= 0).long_name
    }
}
...
class Map {
    ...
    select(address) {
        ...
        $("#farm_address_country").val(address.country);
        $("#farm_address_region").val(address.department + ', ' + address.region);
